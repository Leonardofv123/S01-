use std::io;
 
fn calcular_pontuacao(prova1: f64, prova2: f64, redacao: f64) -> f64 {
    let npt = (prova1 + prova2) / 2.0;

    let pontuacao_final = npt * 0.6 + redacao * 0.4;
 
    if pontuacao_final >= 60.0 {
        println!("Parabens! Candidato aprovado no processo seletivo.");
    } else {
        println!("Infelizmente o candidato nao atingiu a pontuacao minima de aprovacao.");
    }
    pontuacao_final
}
 
fn ler_nota(mensagem: &str) -> f64 {
    loop {
        println!("{}", mensagem);
 
        let mut entrada = String::new();
        io::stdin().read_line(&mut entrada).expect("Erro ao ler");
 
        match entrada.trim().parse() {
            Ok(n) => return n,
            Err(_) => println!("Nota invalida, tente de novo."),
        }
    }
}
 
fn main() {
    let prova1 = ler_nota("Digite a nota da Prova Teorica 1:");
    let prova2 = ler_nota("Digite a nota da Prova Teorica 2:");
    let redacao = ler_nota("Digite a nota da Redacao:");
 
    let pontuacao = calcular_pontuacao(prova1, prova2, redacao);
 
    println!("Pontuacao Final: {:.2}", pontuacao);
}
